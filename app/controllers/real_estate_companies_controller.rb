class RealEstateCompaniesController < ApplicationController
  ...
  def for_realtor
    ...
    respond_to do |format|
      if @real_estate_company.save
        current_user.real_estate_company_id = @real_estate_company.id
        if @current_user.save
          format.html { redirect_to edit_user_registration_path, notice: 'Real estate company was successfully created.' }
        else
          format.html { render :new }
        end
      else
